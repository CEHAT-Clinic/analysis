...
renderPlot({print(ggplot(PAhi_lo[PAhi_lo$type == "high" & PAhi_lo$PM2.5<=300,], aes(x=hour, group = timeofday))+
...
df2 <- data.frame(day = c(daysOfMonth[,"day"], daysOfMonth[,"day"]),city = c(rep(nameOfCity, times = 31),rep("South Gate",times = 31)),PM2.5 = c(daysOfMonth[,"otherCityPM"],daysOfMonth[,"southGatePM"]))

# - boxplot

renderPlotly({
  boxplot <- ggplot(df2, aes(x= city, y=PM2.5))+                     geom_boxplot(outlier.colour="red", outlier.size = 8, fill                 =c("darkolivegreen3","darksalmon"),notch=T) +
...
ttest = stats::t.test(ourData$otherCityPM,ourData$southGatePM)
...
if (ttest$p.value < (.05) && ttest$statistic > 0) {
  print(paste("The",nameOfCity,"data is statistically worse than the South Gate data.",sep=" "))
}
if (ttest$p.value < (.05) && ttest$statistic < 0) {
  print(paste("The South Gate data is statistically worse than the",nameOfCity, "data.",sep = " "))
}
if (ttest$p.value >= (.05)){
