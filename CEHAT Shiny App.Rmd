...
test <- read.csv("december2020_readings.csv")

#cleaned PM2.5 data
SGData <- PurpleAirCEHAT::cleanPA(test)


Sensor1 <- dplyr::filter(SGData, latitude == '33.95094')
Sensor2 <- dplyr::filter(SGData, latitude == '33.9606')
Sensor3 <- dplyr::filter(SGData, latitude == '33.9498')
Sensor4 <- dplyr::filter(SGData, latitude == '33.9619')
Sensor5 <- dplyr::filter(SGData, latitude == '33.9411')
...
# boxplot
boxplot(ourData$otherCityPM, ourData$SouthGatePM, main = "PM2.5 in Other City vs South Gate",
        names = c("Other City","South Gate"), outline=FALSE)

# line chart
x <- strptime(ourData$timestamp, '%Y-%m-%d %H:%M:%S')
plot(x,ourData$southGatePM,main = "otherCity vs South Gate", xlab= "day", ylab= "PM2.5",
     type="l",col="red")
lines(x,ourData$otherCityPM,col="green")

# stacked barchart
counts <- table(ourData$otherCityPM, x)
